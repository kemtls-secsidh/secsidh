...
static long long poly_eval_precomputesize(long long flen)
{
  if (flen <= 2) return 0;
  return flen;
}

static void poly_eval_precompute(fp *precomp,long long flen,const proj *P)
{
  if (flen <= 2) return;
  ...
  for (long long i = 2;i < flen;++i) {
    fp_mul3(&pxpow[i],(const fp*) &pxpow[i-1],&P->x);
    fp_mul3(&pzpow[i],(const fp*) &pzpow[i-1],&P->z);
  }

  fp_copy(precomp[0], pzpow[flen-1]);
  fp_copy(precomp[flen-1], pxpow[flen-1]);
  for (long long i = 1;i < flen-1;++i)
    fp_mul3(&precomp[i],(const fp*) &pxpow[i],(const fp*) &pzpow[flen-1-i]);
}
...
static void poly_eval_postcompute(fp *v,const fp *f,long long flen,const proj *P,const fp *precomp)
{
  assert(flen > 0);
  if (flen == 1) {
    fp_copy(*v, f[0]);
...
// static void poly_eval(fp *v,const fp *f,long long flen,const proj *P)
// {
...
static void poly_pseudoreciprocal(fp *d,fp *r,long long rlen,const fp *m,long long mdeg)
{
  if (mdeg == 0) {
  ...
  fp eps[mdeg];
  poly_mul_low(eps,mdeg,m,mdeg+1,(const fp*) s,top);
  ...
  fp epss[bot];
  poly_mul_high(epss,mdeg+top-bot-1,(const fp*) eps,mdeg,(const fp*) s,top);

  for (long long i = 0;i < bot;++i)
    fp_neg2(&r[i],(const fp*) &epss[i]);
  for (long long i = 0;i < top;++i)
    fp_mul3(&r[i+bot],(const fp*) &s[i],(const fp*) d);

  fp_sq1(d);
}

static long long poly_pseudoremainder_precomputesize(long long glen,long long flen)
{
  assert(flen >= glen);
...
static void poly_pseudoremainder_precompute(fp *precomp,long long glen,long long flen,const fp *m)
{
  assert(flen >= glen);
  if (flen == glen) return;
  long long vlen = flen-glen;
...
static void poly_pseudoremainder_postcompute(fp *g,long long glen,const fp *f,long long flen,const fp *m,const fp *precomp)
{
  assert(flen >= glen);
  ...
  fp qm[glen];
  poly_mul_low(qm,glen,(const fp*) vf,vlen,m,glen+1);

  for (long long i = 0;i < glen;++i) {
    fp_mul3(&g[i],&f[i],d);
    fp_sub2(&g[i], (const fp*) &qm[i]);
  }
}
...
// static void poly_pseudoremainder(fp *g,long long glen,const fp *f,long long flen,const fp *m)
// {
...
static long long poly_multieval_unscaled_precomputesize(long long n,long long flen)
{
  if (n <= 0) return 0;
  if (n == 1)
...
static void poly_multieval_unscaled_precompute(fp *precomp,long long n,long long flen,const fp *P,const fp *T)
{
  if (n <= 0) return;
  ...
  if (n == 1) {
    ...
    fp_neg2(&Pp.x,&P[0]);
    fp_copy(Pp.z, P[1]);
    poly_eval_precompute(precomp,flen,&Pp);
    return;
  }
...
static void poly_multieval_unscaled_postcompute(fp *v,long long n,const fp *f,long long flen,const fp *P,const fp *T,const fp *precomp)
{
  if (n <= 0) return;
  ...
  if (n == 1) {
    ...
    fp_neg2(&Pp.x,&P[0]);
    fp_copy(Pp.z, P[1]);
    poly_eval_postcompute(v,f,flen,&Pp,precomp);
    return;
  }
  ...
  if (n == 2) {
    ...
    precomp += poly_pseudoremainder_precomputesize(n,flen);
    poly_multieval_unscaled_postcompute(v,1,(const fp*) g,n,P,0,precomp);
    precomp += poly_multieval_unscaled_precomputesize(1,n);
    poly_multieval_unscaled_postcompute(v+1,1,(const fp*) g,n,P+2,0,precomp);
    return;
  }
  ...
  if (n == 3) {
    ...
    precomp += poly_pseudoremainder_precomputesize(n,flen);
    poly_multieval_unscaled_postcompute(v,2,(const fp*) g,n,P,T,precomp);
    precomp += poly_multieval_unscaled_precomputesize(2,n);
    poly_multieval_unscaled_postcompute(v+2,1,(const fp*) g,n,P+4,0,precomp);
    return;
  }
  ...
  precomp += poly_pseudoremainder_precomputesize(n,flen);
  poly_multieval_unscaled_postcompute(v,m,(const fp*) g,n,P,T,precomp);
  precomp += poly_multieval_unscaled_precomputesize(m,n);
  poly_multieval_unscaled_postcompute(v+m,n-m,(const fp*) g,n,P+2*m,T+left,precomp);
}

// static void poly_multieval_unscaled(fp *v,long long n,const fp *f,long long flen,const fp *P,const fp *T)
// {
...
static void poly_multieval_scaled(fp *v,long long n,const fp *r,const fp *P,const fp *T)
{
  if (n <= 0) return;
  if (n == 1) {
    fp_copy(v[0], r[0]);
  ...
  if (n == 2) {
    ...
    poly_multieval_scaled(v,1,(const fp*) g,P,0);
    poly_mul_mid(g,1,1,r,2,P,2);
    poly_multieval_scaled(v+1,1,(const fp*) g,P+2,0);
    return;
  }
  if (n == 3) {
    fp g[2];
    poly_mul_mid(g,1,2,r,3,P+4,2);
    poly_multieval_scaled(v,2,(const fp*) g,P,T);
    poly_mul_mid(g,2,1,r,3,T,3);
    poly_multieval_scaled(v+2,1,(const fp*) g,P+4,0);
    return;
  }
  ...
  poly_multieval_scaled(v,m,(const fp*) g,P,T);
  poly_mul_mid(g,m,n-m,r,n,T+left-(m+1),m+1);
  poly_multieval_scaled(v+m,n-m,(const fp*) g,P+2*m,T+left);
}

static long long poly_multieval_chooseunscaled(long long n,long long flen)
{
  /* XXX: tune this */
  if (n <= 1) return 1;
  if (flen <= 1) return 1;
